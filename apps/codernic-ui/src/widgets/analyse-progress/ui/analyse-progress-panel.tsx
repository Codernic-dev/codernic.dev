...
import type { AnalyseProgressState, AnalyseStep } from '../../../entities/kernel/model/types';
...
export function AnalyseProgressPanel({ dataTestId, progress, onDismiss }: AnalyseProgressPanelProps) {
  const { getTestId } = useTestId('analyse-progress-panel');
  
  const rootId = dataTestId || 'analyse-progress-panel';
        ...
        <div style={{ fontSize: '10px', color: '#a1a1aa', fontFamily: 'var(--mono)', marginRight: '6px' }}>
          {isDone ? (
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ⟳ <ElapsedTimer data-testid={getTestId('elapsed-timer')} startedAt={startedAtRef.current} />
            </span>
          )}
        </div>
        {isDone && (
          <Button data-testid={getTestId('dismiss-button')} onClick={onDismiss} variant="secondary" size="xs">
            Dismiss
          </Button>
        )}
      </div>
