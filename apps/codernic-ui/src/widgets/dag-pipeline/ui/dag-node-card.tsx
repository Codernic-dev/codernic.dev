...
import { selectDagNodeById } from '../../../features/dag/store/dag.slice';
import type { NodeStatus } from '../../../entities/kernel/model/types';
...
export function DagNodeCard({ nodeId, level }: DagNodeCardProps) {
  const node = useSelector((state: RootState) => selectDagNodeById(state, nodeId));
  if (!node) return null;
