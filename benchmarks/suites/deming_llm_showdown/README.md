...
This benchmark evaluates physical bare-metal LLM inference and fine-tuning throughput, latency, and memory footprint across real hardware execution targets:
...
- **Zero Simulation / Zero Vaporware**: If a target engine or runtime is missing, it is explicitly logged as `"status": "SKIPPED (NOT_INSTALLED)"`.
- **Sequential Execution**: Engines run strictly one at a time to prevent CPU/GPU core and memory bus contention.
- **Warmup Exclusion**: The first 3 iterations are excluded from average latency and throughput calculations.
- **ALCOA+ Cryptographic Seal**: The final Markdown and JSON reports are cryptographically hashed using SHA-256 and recorded in the audit ledger.
