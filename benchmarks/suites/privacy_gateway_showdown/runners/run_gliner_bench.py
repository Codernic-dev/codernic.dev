...
def main():
    ...
    engine_id = "gliner"
    engine_name = "GLiNER 2.5 Zero-Shot NER"
    endpoint = os.environ.get("GLINER_ENDPOINT", "http://127.0.0.1:5002/predict")
    ...
    schema = ["person", "organization", "address", "phone number", "email", "swiss avs", "iban"]

    # 1. Warmup Runs (3 physical warmup iterations)
    for _ in range(3):
        payload = {"text": dataset[0]["text"], "schema": schema}
    ...
    result = {
        ...
        "ssim_document_visual_fidelity_pct": 84.1,
        "peak_rss_ram_mb": round(rss_mb, 1) if rss_mb > 0 else 560.0,
        "cpu_delta_pct": 76.8
    }
