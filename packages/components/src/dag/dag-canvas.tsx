 ...
 */
export function DagCanvas({ dataTestId,
  nodes,
  edges,
  readonly       = false,
  onNodesChange,
  ...
  onNodeClick,
  className = '',
}: DagCanvasProps & { dataTestId?: string }) {
  
  const { rootId, getTestId } = useTestId('dag-canvas', dataTestId);
  ...
  return (
    <div
      className={[
        ...
        .join(' ')}
    >
      <ReactFlow data-testid={getTestId('react-flow')}
        nodes={layoutedNodes}
        edges={layoutedEdges}
      ...
      >
        <Background data-testid={getTestId('background')} variant={BackgroundVariant.Dots} gap={20} size={1} className="opacity-30" />
        <Controls data-testid={getTestId('controls')} showInteractive={!readonly} />
        {!readonly && <MiniMap data-testid={getTestId('mini-map')} nodeStrokeWidth={2} zoomable pannable />}
      </ReactFlow>
    </div>
  )
}
