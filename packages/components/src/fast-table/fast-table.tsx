...
export function FastTable<T extends Record<string, unknown>>({ dataTestId,
  transporter,
  rows: staticRows,
  columns,
  getRowId,
  ...
  className,
  rowRules,
}: FastTableProps<T> & { dataTestId?: string }) {
  // ── Data source ────────────────────────────────────────────────────────────
  
  const { rootId, getTestId } = useTestId('fast-table', dataTestId);
const rawRows = transporter?.rows ?? staticRows ?? []
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <FastTableContext.Provider data-testid={`${rootId}-fast-table-context.provider`} value={fastTableCtx}>
      <ActiveRowContext.Provider data-testid={`${rootId}-active-row-context.provider`} value={activeRowCtx}>
        <div
          ref={containerRef}
        ...
        >
          {/* Header */}
          <TableHeader data-testid={getTestId('table-header')}
            columns={columns}
            sortState={sortState}
            onSort={handleSort}
            ...
            <div style={{ height: sortedRows.length * rowHeight, position: 'relative' }}>
              {decoratedRows.map(rowState => {
                ...
                if (isActive && form && schema) {
                  return (
                    <ActiveRowFormProvider data-testid={getTestId('active-row-form-provider')}
                      key={`form-${String(id)}`}
                      form={form}
                      schema={schema}
                    >
