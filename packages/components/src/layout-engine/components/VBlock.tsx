...
export function VBlock({ dataTestId, id, parentId, flexRatio }: { id: string, parentId?: string, flexRatio?: number;  dataTestId?: string;
  ...
  const vblockContentNode = (
    <div ref={contentRef} className={`flex-1 flex ${isHorizontal ? 'flex-row' : 'flex-col'} overflow-y-auto overflow-x-hidden min-h-0 min-w-0 gap-1.5 ${isEditMode ? 'p-1.5' : ''}`}>
  ...
  const vblockContent = (
    <VBlockContext.Provider value={{ 
      ...
      <div 
        ...
        className={`flex flex-col relative overflow-y-auto overflow-x-hidden w-full h-full min-h-[50px] min-w-[50px] ${isEditMode ? 'bg-[#111113] transition-all rounded-xl border ' + (isHovered ? 'border-amber-500/50 shadow-[inset_0_0_15px_rgba(245,158,11,0.05)]' : 'border-zinc-800/40') : ''} group/vblock ${!isExpanded && !isEditMode ? 'hidden' : ''} ${isPortalMode ? 'bg-[#09090b] z-[100]' : ''}`}
      >
        <VBlockHeader 
          id={id}
