...
export function useVBlockResize(id: string, ratios: number[], isHorizontal: boolean) {
  ...
  const handleResize = (index: number, clientPos: number) => {
    ...
    if (currentRatios[index] + difference > 0.05 && currentRatios[index + 1] - difference > 0.05) {
       ...
       if (child1) {
         child1.style.flex = `${currentRatios[index] * 100} ${currentRatios[index] * 100} 0%`;
         child1.style.minHeight = '0px';
         child1.style.minWidth = '0px';
       }
       if (child2) {
         child2.style.flex = `${currentRatios[index + 1] * 100} ${currentRatios[index + 1] * 100} 0%`;
         child2.style.minHeight = '0px';
         child2.style.minWidth = '0px';
       }
    }
  };
