...
export function EmptyState({ dataTestId,
  title,
  description,
  action,
  className = '',
  ...
  const { rootId, getTestId } = useTestId('empty-state', dataTestId);
return (
    <div 
    ...
    >
      ...
      <Text data-testid={getTestId('text')} className="font-medium">{title}</Text>

      {description && (
        <Text data-testid={getTestId('text-1')} variant="muted" size="sm">{description}</Text>
      )}
